...
openchamber-mobile() {
  ...
  case "$1" in
    stop|status|logs|update|tunnel|help|-h|--help|-v|--version)
      openchamber "$@"
      return
      ;;
  esac

  local _arg _foreground=0
  for _arg in "$@"; do
    case "$_arg" in
  ...
  local _output _status _port
  _output=$(openchamber --host "$_tailscale_ip" "$@" 2>&1)
  _status=$?
  print -r -- "$_output"

  if (( _status == 0 )); then
    _port=$(print -r -- "$_output" | sed -n 's/.*port \([0-9][0-9]*\).*/\1/p' | head -n 1)
    if [[ -n "$_port" ]]; then
      print -r -- "openchamber-mobile: mobile URL: http://$_tailscale_ip:$_port/"
    fi
  fi
